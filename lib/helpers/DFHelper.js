...
_.extend(DFHelper, {
  ...
  parseWin32: function (stdout, options) {
      ...
      response = _(stdout
        ...
        .slice(1))
          ...
          .map(function (row) {
            ...
            var totalSize = columns[4];
            var freeSize = columns[3];
            var usedSize = totalSize - freeSize;
            var percentage = ( usedSize / totalSize ) * 100;
            var filesystem = columns[2];
            var mount = columns[1];
            var size = format(totalSize, multiplier, 2, displayedPrefixMultiplier);
            var used = format(usedSize, multiplier, 2, displayedPrefixMultiplier);
            var available = format(freeSize, multiplier, 2, displayedPrefixMultiplier);
            var capacity = format(percentage, 1.00);
